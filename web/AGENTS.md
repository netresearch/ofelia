...
- The rate limiter counts ALL requests, static assets included — keep the UI's request budget low; render-only interactions (search, sort) must never refetch. Only `/live` and `/ready` are exempt (a probe answered 429 gets the daemon restarted); `/health` and `/healthz` are token-free but counted, because they answer with the full report — every check, the version and the goroutine count — which is both more work per request and more than a probe needs to know. (`GetHealth` no longer reads MemStats per request; it reports the snapshot the periodic system check takes, which is what makes the `/ready` exemption safe.)
