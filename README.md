...
`--web-address` (default `:8081`) to inspect active job status, removed jobs and view the
active configuration. The UI fetches JSON from `/api/jobs`,  `/api/jobs/removed`
and `/api/config` periodically.
