...
func TestPerformanceRegressionDetection(t *testing.T) {
	...
	// Set performance thresholds (adjust based on expected performance)
	bufferPoolThreshold := 100 * time.Millisecond  // 10 μs per operation
	circuitBreakerThreshold := 50 * time.Millisecond  // 5 μs per operation
	metricsThreshold := 10 * time.Millisecond  // 1 μs per operation
