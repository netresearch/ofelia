...
- Exclude a rule on a path: `POST api/settings/set` with `key=sonar.issue.ignore.multicriteria` (verify via `api/settings/values`). Issue dispositions: `api/issues/do_transition` (`falsepositive`, `accept`, `confirm`, `resolve`). Hotspots: `api/hotspots/change_status` — SonarCloud only accepts `REVIEWED` + `SAFE`/`FIXED`, no ACKNOWLEDGED.
